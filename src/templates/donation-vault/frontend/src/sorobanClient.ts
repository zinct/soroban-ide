...
const DEPLOY_HINT = "Deploy contracts/donation_vault in the Deploy panel, set VITE_CONTRACT_ID, then Rebuild preview.";
...
const formatRpcError = (err: unknown, context: string): Error => {
  ...
  if (/non-existent contract function/i.test(message)) {
